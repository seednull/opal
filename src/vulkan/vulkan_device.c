...
static void vulkan_cmdPipelineBarrier(Vulkan_Device *device_ptr, Vulkan_CommandBuffer *command_buffer_ptr, const Opal_PassBarriersDesc *barriers)
{
	assert(device_ptr);
	...
	for (uint32_t i = 0; i < barriers->num_barriers; ++i)
	{
		...
		Vulkan_Fence *fence_ptr = NULL;
		if (opal_barrier->fence != OPAL_NULL_HANDLE)
		{
			fence_ptr = (Vulkan_Fence *)opal_poolGetElement(&device_ptr->fences, (Opal_PoolHandle)opal_barrier->fence);
			assert(fence_ptr);
		}

		VkPipelineStageFlagBits src_stages = vulkan_helperToPipelineWaitStages(opal_barrier->wait_stages);
		VkPipelineStageFlagBits dst_stages = vulkan_helperToPipelineBlockStages(opal_barrier->block_stages);

		opal_bumpReset(&device_ptr->bump);
		uint32_t buffer_barriers_offset = opal_bumpAlloc(&device_ptr->bump, sizeof(VkBufferMemoryBarrier) * opal_barrier->num_buffer_transitions);
		uint32_t texture_barriers_offset = opal_bumpAlloc(&device_ptr->bump, sizeof(VkImageMemoryBarrier) * opal_barrier->num_texture_transitions);

		VkBufferMemoryBarrier *buffer_barriers = NULL;
		VkImageMemoryBarrier *texture_barriers = NULL;

		if (opal_barrier->num_buffer_transitions > 0)
		{
			buffer_barriers = (VkBufferMemoryBarrier *)(device_ptr->bump.data + buffer_barriers_offset);
			memset(buffer_barriers, 0, sizeof(VkBufferMemoryBarrier) * opal_barrier->num_buffer_transitions);
			...
			for (uint32_t j = 0; j < opal_barrier->num_buffer_transitions; ++j)
			{
				...
				VkBufferMemoryBarrier *vulkan_barrier = &buffer_barriers[j];

				Vulkan_Buffer *buffer_ptr = (Vulkan_Buffer *)opal_poolGetElement(&device_ptr->buffers, (Opal_PoolHandle)opal_transition->buffer);
				assert(buffer_ptr);

				vulkan_barrier->sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER;

				if (opal_barrier->wait_stages != OPAL_BARRIER_STAGE_NONE)
					vulkan_barrier->srcAccessMask = vulkan_helperToBufferAccessMask(buffer_ptr->usage, opal_transition->state_before);

				if (opal_barrier->block_stages != OPAL_BARRIER_STAGE_NONE)
					vulkan_barrier->dstAccessMask = vulkan_helperToBufferAccessMask(buffer_ptr->usage, opal_transition->state_after);

				vulkan_barrier->srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
				vulkan_barrier->dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
				vulkan_barrier->buffer = buffer_ptr->buffer;
				vulkan_barrier->size = VK_WHOLE_SIZE;
			}
		}

		if (opal_barrier->num_texture_transitions > 0)
		{
			texture_barriers = (VkImageMemoryBarrier *)(device_ptr->bump.data + texture_barriers_offset);
			memset(texture_barriers, 0, sizeof(VkImageMemoryBarrier) * opal_barrier->num_texture_transitions);
			...
			for (uint32_t j = 0; j < opal_barrier->num_texture_transitions; ++j)
			{
				...
				VkImageMemoryBarrier *vulkan_barrier = &texture_barriers[j];

				Vulkan_ImageView *image_view_ptr = (Vulkan_ImageView *)opal_poolGetElement(&device_ptr->image_views, (Opal_PoolHandle)opal_transition->texture_view);
				assert(image_view_ptr);

				vulkan_barrier->sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;

				if (opal_barrier->wait_stages != OPAL_BARRIER_STAGE_NONE)
					vulkan_barrier->srcAccessMask = vulkan_helperToImageAccessMask(image_view_ptr->usage, opal_transition->state_before);
				
				if (opal_barrier->block_stages != OPAL_BARRIER_STAGE_NONE)
					vulkan_barrier->dstAccessMask = vulkan_helperToImageAccessMask(image_view_ptr->usage, opal_transition->state_after);

				vulkan_barrier->oldLayout = vulkan_helperToImageLayout(image_view_ptr->usage, opal_transition->state_before, image_view_ptr->format);
				vulkan_barrier->newLayout = vulkan_helperToImageLayout(image_view_ptr->usage, opal_transition->state_after, image_view_ptr->format);
				vulkan_barrier->srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
				vulkan_barrier->dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
				vulkan_barrier->image = image_view_ptr->image;
				vulkan_barrier->subresourceRange.aspectMask = image_view_ptr->aspect_mask;
				vulkan_barrier->subresourceRange.baseMipLevel = image_view_ptr->base_mip;
				vulkan_barrier->subresourceRange.levelCount = image_view_ptr->num_mips;
				vulkan_barrier->subresourceRange.baseArrayLayer = image_view_ptr->base_layer;
				vulkan_barrier->subresourceRange.layerCount = image_view_ptr->num_layers;
			}
		}
			...
			switch (opal_barrier->fence_op)
			{
				...
				case OPAL_FENCE_OP_END:
				{
					device_ptr->vk.vkCmdWaitEvents(
						...
						0, NULL,
						opal_barrier->num_buffer_transitions, buffer_barriers,
						opal_barrier->num_texture_transitions, texture_barriers
					);
				}
				break;
		...
		{
			device_ptr->vk.vkCmdPipelineBarrier(
				command_buffer_ptr->command_buffer,
...
static void vulkan_cmdStageBarrier(Vulkan_Device *device_ptr, Vulkan_CommandBuffer *command_buffer_ptr, const Opal_MemoryBarrierDesc *barriers, VkPipelineStageFlagBits stage)
{
	...
	VkAccessFlagBits src_access = VK_ACCESS_SHADER_WRITE_BIT;
	VkAccessFlagBits dst_access = VK_ACCESS_SHADER_READ_BIT;

	opal_bumpReset(&device_ptr->bump);
	uint32_t buffer_barriers_offset = opal_bumpAlloc(&device_ptr->bump, sizeof(VkBufferMemoryBarrier) * barriers->num_buffers);
	uint32_t texture_barriers_offset = opal_bumpAlloc(&device_ptr->bump, sizeof(VkImageMemoryBarrier) * barriers->num_textures);

	VkBufferMemoryBarrier *buffer_barriers = NULL;
	VkImageMemoryBarrier *texture_barriers = NULL;

	if (barriers->num_buffers > 0)
	{
		buffer_barriers = (VkBufferMemoryBarrier *)(device_ptr->bump.data + buffer_barriers_offset);
		memset(buffer_barriers, 0, sizeof(VkBufferMemoryBarrier) * barriers->num_buffers);

		for (uint32_t i = 0; i < barriers->num_buffers; ++i)
		{
			VkBufferMemoryBarrier *vulkan_barrier = &buffer_barriers[i];

			Vulkan_Buffer *buffer_ptr = (Vulkan_Buffer *)opal_poolGetElement(&device_ptr->buffers, (Opal_PoolHandle)barriers->buffers[i]);
			assert(buffer_ptr);

			vulkan_barrier->sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER;
			vulkan_barrier->srcAccessMask = src_access;
			vulkan_barrier->dstAccessMask = dst_access;
			vulkan_barrier->srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
			vulkan_barrier->dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
			vulkan_barrier->buffer = buffer_ptr->buffer;
			vulkan_barrier->size = VK_WHOLE_SIZE;
	...
	if (barriers->num_textures > 0)
	{
		texture_barriers = (VkImageMemoryBarrier *)(device_ptr->bump.data + texture_barriers_offset);
		memset(texture_barriers, 0, sizeof(VkImageMemoryBarrier) * barriers->num_textures);

		for (uint32_t i = 0; i < barriers->num_textures; ++i)
		{
			VkImageMemoryBarrier *vulkan_barrier = &texture_barriers[i];

			Vulkan_ImageView *image_view_ptr = (Vulkan_ImageView *)opal_poolGetElement(&device_ptr->image_views, (Opal_PoolHandle)barriers->textures[i]);
			assert(image_view_ptr);
			...
			vulkan_barrier->srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
			vulkan_barrier->dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
			vulkan_barrier->oldLayout = VK_IMAGE_LAYOUT_GENERAL;
			vulkan_barrier->newLayout = VK_IMAGE_LAYOUT_GENERAL;
			vulkan_barrier->image = image_view_ptr->image;
			vulkan_barrier->subresourceRange.aspectMask = image_view_ptr->aspect_mask;
			vulkan_barrier->subresourceRange.baseMipLevel = image_view_ptr->base_mip;
			vulkan_barrier->subresourceRange.levelCount = image_view_ptr->num_mips;
	...
	device_ptr->vk.vkCmdPipelineBarrier(
		...
		src_stages, dst_stages, 0,
		0, NULL,
		barriers->num_buffers, buffer_barriers,
		barriers->num_textures, texture_barriers
	);
}

static void vulkan_cmdSetDescriptorSet(Vulkan_Device *device_ptr, Vulkan_CommandBuffer *command_buffer_ptr, VkPipelineBindPoint vulkan_bind_point, uint32_t index, Opal_DescriptorSet descriptor_set, uint32_t num_dynamic_offsets, const uint32_t *dynamic_offsets)
{
	assert(device_ptr);
	assert(command_buffer_ptr);
...
static void vulkan_destroyFence(Vulkan_Device *device_ptr, Vulkan_Fence *fence_ptr)
{
	assert(device_ptr);
	assert(fence_ptr);

	device_ptr->vk.vkDestroyEvent(device_ptr->device, fence_ptr->event, NULL);
}
...
static void vulkan_destroyGraphicsPipeline(Vulkan_Device *device_ptr, Vulkan_GraphicsPipeline *pipeline_ptr)
{
	assert(device_ptr);
	assert(pipeline_ptr);
...
static void vulkan_destroyComputePipeline(Vulkan_Device *device_ptr, Vulkan_ComputePipeline *pipeline_ptr)
{
	assert(device_ptr);
	assert(pipeline_ptr);
...
static void vulkan_destroyRaytracePipeline(Vulkan_Device *device_ptr, Vulkan_RaytracePipeline *pipeline_ptr)
{
	assert(device_ptr);
	assert(pipeline_ptr);
...
static Opal_Result vulkan_deviceCreateFence(Opal_Device this, Opal_Fence *fence)
{
	assert(this);
	assert(fence);
	...
	VkEvent vulkan_event = VK_NULL_HANDLE;

	VkEventCreateInfo event_info = {0};
	event_info.sType = VK_STRUCTURE_TYPE_EVENT_CREATE_INFO;

	VkResult vulkan_result = device_ptr->vk.vkCreateEvent(vulkan_device, &event_info, NULL, &vulkan_event);
	if (vulkan_result != VK_SUCCESS)
		return OPAL_VULKAN_ERROR;
...
static Opal_Result vulkan_deviceCreateTextureView(Opal_Device this, const Opal_TextureViewDesc *desc, Opal_TextureView *texture_view)
{
	...
	result.usage = image_ptr->usage;
	result.format = image_ptr->format;
...
static Opal_Result vulkan_deviceCreateShaderBindingTable(Opal_Device this, Opal_RaytracePipeline pipeline, Opal_ShaderBindingTable *shader_binding_table)
{
	assert(this);
	assert(pipeline);
...
static Opal_Result vulkan_deviceCreateCommandBuffer(Opal_Device this, Opal_CommandAllocator command_allcoator, Opal_CommandBuffer *command_buffer)
{
	...
	result.command_allocator = command_allcoator;
...
static Opal_Result vulkan_deviceCreateMeshletPipeline(Opal_Device this, const Opal_MeshletPipelineDesc *desc, Opal_GraphicsPipeline *pipeline)
{
	assert(this);
	assert(desc);
...
static Opal_Result vulkan_deviceCreateComputePipeline(Opal_Device this, const Opal_ComputePipelineDesc *desc, Opal_ComputePipeline *pipeline)
{
	...
	Vulkan_ComputePipeline result = {0};
	result.pipeline = vulkan_pipeline;
...
static Opal_Result vulkan_deviceCreateRaytracePipeline(Opal_Device this, const Opal_RaytracePipelineDesc *desc, Opal_RaytracePipeline *pipeline)
{
	...
	Vulkan_RaytracePipeline result = {0};
	result.pipeline = vulkan_pipeline;
	...
	*pipeline = (Opal_RaytracePipeline)opal_poolAddElement(&device_ptr->raytrace_pipelines, &result);
	return OPAL_SUCCESS;
}
...
static Opal_Result vulkan_deviceCreateSwapchain(Opal_Device this, const Opal_SwapchainDesc *desc, Opal_Swapchain *swapchain)
{
	...
	for (uint32_t i = 0; i < num_images; ++i)
	{
		...
		result.usage = desc->usage;
		result.format = desc->format.texture_format;
...
static Opal_Result vulkan_deviceDestroyFence(Opal_Device this, Opal_Fence fence)
{
	assert(this);
	assert(fence);
	...
	Vulkan_Device *device_ptr = (Vulkan_Device *)this;
	Vulkan_Fence *fence_ptr = (Vulkan_Fence *)opal_poolGetElement(&device_ptr->fences, handle);
	assert(fence_ptr);

	opal_poolRemoveElement(&device_ptr->fences, handle);

	vulkan_destroyFence(device_ptr, fence_ptr);
	return OPAL_SUCCESS;
}
...
static Opal_Result vulkan_deviceDestroyCommandBuffer(Opal_Device this, Opal_CommandBuffer command_buffer)
{
	...
	Vulkan_CommandAllocator *command_allocator_ptr = (Vulkan_CommandAllocator *)opal_poolGetElement(&device_ptr->command_allocators, (Opal_PoolHandle)command_buffer_ptr->command_allocator);
	assert(command_allocator_ptr);
...
static Opal_Result vulkan_deviceDestroyGraphicsPipeline(Opal_Device this, Opal_GraphicsPipeline pipeline)
{
	...
	Vulkan_Device *device_ptr = (Vulkan_Device *)this;
	Vulkan_GraphicsPipeline *pipeline_ptr = (Vulkan_GraphicsPipeline *)opal_poolGetElement(&device_ptr->graphics_pipelines, handle);
	assert(pipeline_ptr);

	opal_poolRemoveElement(&device_ptr->graphics_pipelines, handle);

	vulkan_destroyGraphicsPipeline(device_ptr, pipeline_ptr);
	return OPAL_SUCCESS;
}
...
static Opal_Result vulkan_deviceDestroyComputePipeline(Opal_Device this, Opal_ComputePipeline pipeline)
{
	...
	Vulkan_Device *device_ptr = (Vulkan_Device *)this;
	Vulkan_ComputePipeline *pipeline_ptr = (Vulkan_ComputePipeline *)opal_poolGetElement(&device_ptr->compute_pipelines, handle);
	assert(pipeline_ptr);

	opal_poolRemoveElement(&device_ptr->compute_pipelines, handle);

	vulkan_destroyComputePipeline(device_ptr, pipeline_ptr);
	return OPAL_SUCCESS;
}
...
static Opal_Result vulkan_deviceDestroyRaytracePipeline(Opal_Device this, Opal_RaytracePipeline pipeline)
{
	...
	Vulkan_Device *device_ptr = (Vulkan_Device *)this;
	Vulkan_RaytracePipeline *pipeline_ptr = (Vulkan_RaytracePipeline *)opal_poolGetElement(&device_ptr->raytrace_pipelines, handle);
	assert(pipeline_ptr);

	opal_poolRemoveElement(&device_ptr->raytrace_pipelines, handle);

	vulkan_destroyRaytracePipeline(device_ptr, pipeline_ptr);
	return OPAL_SUCCESS;
}
...
static Opal_Result vulkan_deviceDestroy(Opal_Device this)
{
...

	{
		...
		while (head != OPAL_POOL_HANDLE_NULL)
		{
			Vulkan_RaytracePipeline *pipeline_ptr = (Vulkan_RaytracePipeline *)opal_poolGetElementByIndex(&ptr->raytrace_pipelines, head);
			vulkan_destroyRaytracePipeline(ptr, pipeline_ptr);
...

	{
		...
		while (head != OPAL_POOL_HANDLE_NULL)
		{
			Vulkan_ComputePipeline *pipeline_ptr = (Vulkan_ComputePipeline *)opal_poolGetElementByIndex(&ptr->compute_pipelines, head);
			vulkan_destroyComputePipeline(ptr, pipeline_ptr);
...

	{
		...
		while (head != OPAL_POOL_HANDLE_NULL)
		{
			Vulkan_GraphicsPipeline *pipeline_ptr = (Vulkan_GraphicsPipeline *)opal_poolGetElementByIndex(&ptr->graphics_pipelines, head);
			vulkan_destroyGraphicsPipeline(ptr, pipeline_ptr);
...

	{
		...
		while (head != OPAL_POOL_HANDLE_NULL)
		{
			Vulkan_Fence *fence_ptr = (Vulkan_Fence *)opal_poolGetElementByIndex(&ptr->fences, head);
			vulkan_destroyFence(ptr, fence_ptr);
...
static Opal_Result vulkan_deviceBuildShaderBindingTable(Opal_Device this, Opal_ShaderBindingTable shader_binding_table, const Opal_ShaderBindingTableBuildDesc *desc)
{
	...
	Vulkan_RaytracePipeline *pipeline_ptr = (Vulkan_RaytracePipeline *)opal_poolGetElement(&device_ptr->raytrace_pipelines, (Opal_PoolHandle)shader_binding_table_ptr->pipeline);
	assert(pipeline_ptr);
...
static Opal_Result vulkan_deviceBuildAccelerationStructureInstanceBuffer(Opal_Device this, const Opal_AccelerationStructureInstanceBufferBuildDesc *desc)
{
	...
	for (uint32_t i = 0; i < desc->num_instances; ++i)
	{
		...
		vulkan_instance->instanceShaderBindingTableRecordOffset = opal_instance->intersection_index_offset;
...
static Opal_Result vulkan_deviceWriteBuffer(Opal_Device this, Opal_Buffer buffer, uint64_t offset, const void *data, uint64_t size)
{
	...
	uint8_t *ptr = NULL;
	Opal_Result result = vulkan_deviceMapBuffer(this, buffer, &ptr);
	if (result != OPAL_SUCCESS)
		return result;

	memcpy(ptr + offset, data, size);

	return vulkan_deviceUnmapBuffer(this, buffer);
}
...
static Opal_Result vulkan_deviceCmdBeginGraphicsPass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_FramebufferDesc *framebuffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	assert(framebuffer);
	...
	if (barriers != NULL)
		vulkan_cmdPipelineBarrier(device_ptr, command_buffer_ptr, barriers);
	...
	uint32_t color_offset = opal_bumpAlloc(&device_ptr->bump, sizeof(VkRenderingAttachmentInfo) * framebuffer->num_color_attachments);
	...
	if (framebuffer->num_color_attachments > 0)
	{
		...
		memset(vulkan_color_attachments, 0, sizeof(VkRenderingAttachmentInfo) * framebuffer->num_color_attachments);
	...
	if (framebuffer->depth_stencil_attachment)
	{
		...
		vulkan_depth_stencil_attachment->loadOp = vulkan_helperToLoadOp(opal_attachment->load_op);
		vulkan_depth_stencil_attachment->storeOp = vulkan_helperToStoreOp(opal_attachment->store_op);

		assert(sizeof(VkClearValue) == sizeof(Opal_ClearValue));
		memcpy(&vulkan_depth_stencil_attachment->clearValue, &opal_attachment->clear_value, sizeof(VkClearValue));
	}
	...
	command_buffer_ptr->pass = VULKAN_PASS_TYPE_GRAPHICS;
	return OPAL_SUCCESS;
}

static Opal_Result vulkan_deviceCmdGraphicsSetPipelineLayout(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_PipelineLayout pipeline_layout)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result vulkan_deviceCmdGraphicsSetPipeline(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_GraphicsPipeline pipeline)
{
	assert(this);
	assert(command_buffer);
	...
	Vulkan_GraphicsPipeline *pipeline_ptr = (Vulkan_GraphicsPipeline *)opal_poolGetElement(&device_ptr->graphics_pipelines, (Opal_PoolHandle)pipeline);
	assert(pipeline_ptr);
...
static Opal_Result vulkan_deviceCmdGraphicsSetDescriptorSet(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t index, Opal_DescriptorSet descriptor_set, uint32_t num_dynamic_offsets, const uint32_t *dynamic_offsets)
{
	assert(this);
	assert(command_buffer);
	...
	vulkan_cmdSetDescriptorSet(device_ptr, command_buffer_ptr, VK_PIPELINE_BIND_POINT_GRAPHICS, index, descriptor_set, num_dynamic_offsets, dynamic_offsets);
	return OPAL_SUCCESS;
}
...
static Opal_Result vulkan_deviceCmdGraphicsSetIndexBuffer(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_IndexBufferView index_buffer)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result vulkan_deviceCmdGraphicsSetViewport(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_Viewport viewport)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result vulkan_deviceCmdGraphicsSetScissor(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t x, uint32_t y, uint32_t width, uint32_t height)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result vulkan_deviceCmdGraphicsDraw(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t num_vertices, uint32_t num_instances, uint32_t base_vertex, uint32_t base_instance)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result vulkan_deviceCmdGraphicsDrawIndexed(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t num_indices, uint32_t num_instances, uint32_t base_index, int32_t vertex_offset, uint32_t base_instance)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result vulkan_deviceCmdGraphicsMeshletDispatch(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t num_threadgroups_x, uint32_t num_threadgroups_y, uint32_t num_threadgroups_z)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result vulkan_deviceCmdEndGraphicsPass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result vulkan_deviceCmdBeginComputePass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	...
	command_buffer_ptr->pass = VULKAN_PASS_TYPE_COMPUTE;
	return OPAL_SUCCESS;
}

static Opal_Result vulkan_deviceCmdComputeSetPipelineLayout(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_PipelineLayout pipeline_layout)
{
	assert(this);
	assert(command_buffer);
	...
	Vulkan_CommandBuffer *command_buffer_ptr = (Vulkan_CommandBuffer *)opal_poolGetElement(&device_ptr->command_buffers, (Opal_PoolHandle)command_buffer);
	assert(command_buffer_ptr);
	assert(command_buffer_ptr->pass == VULKAN_PASS_TYPE_COMPUTE);

	command_buffer_ptr->pipeline_layout = pipeline_layout;
	return OPAL_SUCCESS;
}

static Opal_Result vulkan_deviceCmdComputeSetPipeline(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_GraphicsPipeline pipeline)
{
	assert(this);
	assert(command_buffer);
	...
	Vulkan_ComputePipeline *pipeline_ptr = (Vulkan_ComputePipeline *)opal_poolGetElement(&device_ptr->compute_pipelines, (Opal_PoolHandle)pipeline);
	assert(pipeline_ptr);
...
static Opal_Result vulkan_deviceCmdComputeSetDescriptorSet(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t index, Opal_DescriptorSet descriptor_set, uint32_t num_dynamic_offsets, const uint32_t *dynamic_offsets)
{
	assert(this);
	assert(command_buffer);
	...
	vulkan_cmdSetDescriptorSet(device_ptr, command_buffer_ptr, VK_PIPELINE_BIND_POINT_COMPUTE, index, descriptor_set, num_dynamic_offsets, dynamic_offsets);
	return OPAL_SUCCESS;
}

static Opal_Result vulkan_deviceCmdComputeMemoryBarrier(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_MemoryBarrierDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	assert(barriers);
	...
	vulkan_cmdStageBarrier(device_ptr, command_buffer_ptr, barriers, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT);
	return OPAL_SUCCESS;
}
...
static Opal_Result vulkan_deviceCmdEndComputePass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result vulkan_deviceCmdBeginRaytracePass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	...
	command_buffer_ptr->pass = VULKAN_PASS_TYPE_RAYTRACE;
	return OPAL_SUCCESS;
}

static Opal_Result vulkan_deviceCmdRaytraceSetPipelineLayout(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_PipelineLayout pipeline_layout)
{
	assert(this);
	assert(command_buffer);
	...
	Vulkan_CommandBuffer *command_buffer_ptr = (Vulkan_CommandBuffer *)opal_poolGetElement(&device_ptr->command_buffers, (Opal_PoolHandle)command_buffer);
	assert(command_buffer_ptr);
	assert(command_buffer_ptr->pass == VULKAN_PASS_TYPE_RAYTRACE);

	command_buffer_ptr->pipeline_layout = pipeline_layout;
	return OPAL_SUCCESS;
}

static Opal_Result vulkan_deviceCmdRaytraceSetPipeline(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_GraphicsPipeline pipeline)
{
	assert(this);
	assert(command_buffer);
	...
	Vulkan_RaytracePipeline *pipeline_ptr = (Vulkan_RaytracePipeline *)opal_poolGetElement(&device_ptr->raytrace_pipelines, (Opal_PoolHandle)pipeline);
	assert(pipeline_ptr);

	device_ptr->vk.vkCmdBindPipeline(command_buffer_ptr->command_buffer, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, pipeline_ptr->pipeline);
	return OPAL_SUCCESS;
}

static Opal_Result vulkan_deviceCmdRaytraceSetDescriptorSet(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t index, Opal_DescriptorSet descriptor_set, uint32_t num_dynamic_offsets, const uint32_t *dynamic_offsets)
{
	assert(this);
	assert(command_buffer);
	...
	vulkan_cmdSetDescriptorSet(device_ptr, command_buffer_ptr, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, index, descriptor_set, num_dynamic_offsets, dynamic_offsets);
	return OPAL_SUCCESS;
}

static Opal_Result vulkan_deviceCmdRaytraceSetShaderBindingTable(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_ShaderBindingTable shader_binding_table)
{
	assert(this);
	assert(command_buffer);
	assert(shader_binding_table);
...
static Opal_Result vulkan_deviceCmdRaytraceMemoryBarrier(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_MemoryBarrierDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	assert(barriers);
	...
	vulkan_cmdStageBarrier(device_ptr, command_buffer_ptr, barriers, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR);
	return OPAL_SUCCESS;
}
...
static Opal_Result vulkan_deviceCmdEndRaytracePass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result vulkan_deviceCmdBeginCopyPass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	...
	command_buffer_ptr->pass = VULKAN_PASS_TYPE_COPY;
	return OPAL_SUCCESS;
}
...
static Opal_Result vulkan_deviceCmdCopyBufferToBuffer(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_Buffer src_buffer, uint64_t src_offset, Opal_Buffer dst_buffer, uint64_t dst_offset, uint64_t size)
{
	...
	Vulkan_CommandBuffer *command_buffer_ptr = (Vulkan_CommandBuffer *)opal_poolGetElement(&device_ptr->command_buffers, (Opal_PoolHandle)command_buffer);
	assert(command_buffer_ptr);
	assert(command_buffer_ptr->pass == VULKAN_PASS_TYPE_COPY);
...
static Opal_Result vulkan_deviceCmdEndCopyPass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result vulkan_deviceCmdBeginAccelerationStructurePass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	...
	command_buffer_ptr->pass = VULKAN_PASS_TYPE_ACCELERATION_STRUCTURE;

	return OPAL_SUCCESS;
}

static Opal_Result vulkan_deviceCmdAccelerationStructureBuild(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_AccelerationStructureBuildDesc *desc)
{
	assert(this);
	assert(command_buffer);
	assert(desc);
	...
	Vulkan_CommandBuffer *command_buffer_ptr = (Vulkan_CommandBuffer *)opal_poolGetElement(&device_ptr->command_buffers, (Opal_PoolHandle)command_buffer);
	assert(command_buffer_ptr);
	assert(command_buffer_ptr->pass == VULKAN_PASS_TYPE_ACCELERATION_STRUCTURE);
	...
	if (desc->type == OPAL_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL)
	{
		for (uint32_t j = 0; j < build_info.geometryCount; ++j)
		{
			...
			VkAccelerationStructureGeometryKHR *geometry = &entries[j];
			VkAccelerationStructureBuildRangeInfoKHR *build_range = &build_ranges[j];
	...
	{
		VkAccelerationStructureGeometryKHR *geometry = &entries[0];
		VkAccelerationStructureBuildRangeInfoKHR *build_range = &build_ranges[0];
		const Opal_AccelerationStructureBuildInputTopLevel *input = &desc->input.top_level;
	...
	device_ptr->vk.vkCmdBuildAccelerationStructuresKHR(vulkan_command_buffer, 1, &build_info, &build_ranges);

	return OPAL_SUCCESS;
}

static Opal_Result vulkan_deviceCmdAccelerationStructureCopy(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_AccelerationStructureCopyDesc *desc)
{
	assert(this);
	assert(command_buffer);
	assert(desc);
	...
	const Vulkan_AccelerationStructure *src_ptr = opal_poolGetElement(&device_ptr->acceleration_structures, (Opal_PoolHandle)desc->src_acceleration_structure);
	assert(src_ptr);

	const Vulkan_AccelerationStructure *dst_ptr = opal_poolGetElement(&device_ptr->acceleration_structures, (Opal_PoolHandle)desc->dst_acceleration_structure);
	assert(dst_ptr);
	...
	copy_info.mode = vulkan_helperToAccelerationStructureCopyMode(desc->copy_mode);
...
static Opal_Result vulkan_deviceCmdEndAccelerationStructurePass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
...
static Opal_DeviceTable device_vtbl =
{
	...
	vulkan_deviceCmdGraphicsSetPipelineLayout,
	vulkan_deviceCmdGraphicsSetPipeline,
	vulkan_deviceCmdGraphicsSetDescriptorSet,
	vulkan_deviceCmdGraphicsSetVertexBuffers,
	vulkan_deviceCmdGraphicsSetIndexBuffer,
	vulkan_deviceCmdGraphicsSetViewport,
	vulkan_deviceCmdGraphicsSetScissor,
	vulkan_deviceCmdGraphicsDraw,
	vulkan_deviceCmdGraphicsDrawIndexed,
	vulkan_deviceCmdGraphicsMeshletDispatch,
	vulkan_deviceCmdEndGraphicsPass,

	vulkan_deviceCmdBeginComputePass,
	vulkan_deviceCmdComputeSetPipelineLayout,
	vulkan_deviceCmdComputeSetPipeline,
	vulkan_deviceCmdComputeSetDescriptorSet,
	vulkan_deviceCmdComputeMemoryBarrier,
	vulkan_deviceCmdComputeDispatch,
	vulkan_deviceCmdEndComputePass,

	vulkan_deviceCmdBeginRaytracePass,
	vulkan_deviceCmdRaytraceSetPipelineLayout,
	vulkan_deviceCmdRaytraceSetPipeline,
	vulkan_deviceCmdRaytraceSetDescriptorSet,
	vulkan_deviceCmdRaytraceSetShaderBindingTable,
	vulkan_deviceCmdRaytraceMemoryBarrier,
	...
	vulkan_deviceCmdBeginAccelerationStructurePass,
	vulkan_deviceCmdAccelerationStructureBuild,
	vulkan_deviceCmdAccelerationStructureCopy,
	vulkan_deviceCmdEndAccelerationStructurePass,
};
...
Opal_Result vulkan_deviceInitialize(Vulkan_Device *device_ptr, Vulkan_Instance *instance_ptr, VkPhysicalDevice physical_device, VkDevice device)
{
	...
	opal_poolInitialize(&device_ptr->fences, sizeof(Vulkan_Fence), 32);
	...
	opal_poolInitialize(&device_ptr->graphics_pipelines, sizeof(Vulkan_GraphicsPipeline), 32);
	opal_poolInitialize(&device_ptr->compute_pipelines, sizeof(Vulkan_ComputePipeline), 32);
	opal_poolInitialize(&device_ptr->raytrace_pipelines, sizeof(Vulkan_RaytracePipeline), 32);
