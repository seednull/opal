...
/*
 */
static Opal_Result directx12_deviceAllocateMemory(DirectX12_Device *device_ptr, const DirectX12_AllocationDesc *desc, DirectX12_Allocation *allocation)
{
	assert(device_ptr);
...
static Opal_Result directx12_createBuffer(DirectX12_Device *device_ptr, const D3D12_RESOURCE_DESC *buffer_info, D3D12_RESOURCE_STATES initial_state, Opal_AllocationMemoryType memory_type, Opal_AllocationHint hint, ID3D12Resource **d3d12_buffer, DirectX12_Allocation *allocation)
{
	assert(device_ptr);
	assert(buffer_info);
	assert(d3d12_buffer);
	...
	allocation_desc.allocation_type = memory_type;
...
static void directx12_cmdResourceBarrier(DirectX12_Device *device_ptr, DirectX12_CommandBuffer *command_buffer_ptr, const Opal_PassBarriersDesc *barriers)
{
	assert(device_ptr);
	assert(command_buffer_ptr);
	assert(command_buffer_ptr->pass == DIRECTX12_PASS_TYPE_NONE);
	assert(barriers);

	uint32_t num_barriers = 0;
	for (uint32_t i = 0; i < barriers->num_barriers; ++i)
	{
		const Opal_BarrierDesc *opal_barrier = &barriers->barriers[i];

		num_barriers += opal_barrier->num_buffer_transitions;

		for (uint32_t j = 0; j < opal_barrier->num_texture_transitions; ++j)
		{
			const Opal_TextureTransitionDesc *opal_transition = &opal_barrier->texture_transitions[j];

			DirectX12_TextureView *texture_view_ptr = (DirectX12_TextureView *)opal_poolGetElement(&device_ptr->texture_views, (Opal_PoolHandle)opal_transition->texture_view);
			assert(texture_view_ptr);

			num_barriers += texture_view_ptr->num_subresources;
		}
	}

	opal_bumpReset(&device_ptr->bump);
	opal_bumpAlloc(&device_ptr->bump, sizeof(D3D12_RESOURCE_BARRIER) * num_barriers);

	D3D12_RESOURCE_BARRIER *d3d12_barriers = (D3D12_RESOURCE_BARRIER *)(device_ptr->bump.data);
	memset(d3d12_barriers, 0, sizeof(D3D12_RESOURCE_BARRIER) * num_barriers);
	...
	for (uint32_t i = 0; i < barriers->num_barriers; ++i)
	{
		...
		D3D12_RESOURCE_BARRIER_FLAGS flags = 0;
		if (opal_barrier->fence != OPAL_NULL_HANDLE)
		{
			assert(opal_poolGetElement(&device_ptr->fences, (Opal_PoolHandle)opal_barrier->fence) != NULL);
		...
		for (uint32_t j = 0; j < opal_barrier->num_buffer_transitions; ++j)
		{
			const Opal_BufferTransitionDesc *opal_transition = &opal_barrier->buffer_transitions[j];

			DirectX12_Buffer *buffer_ptr = (DirectX12_Buffer *)opal_poolGetElement(&device_ptr->buffers, (Opal_PoolHandle)opal_transition->buffer);
			assert(buffer_ptr);

			D3D12_RESOURCE_STATES state_before = directx12_helperToBufferState(buffer_ptr->memory_type, buffer_ptr->usage, opal_transition->state_before);
			D3D12_RESOURCE_STATES state_after = directx12_helperToBufferState(buffer_ptr->memory_type, buffer_ptr->usage, opal_transition->state_after);

			if (state_before == state_after)
				continue;

			D3D12_RESOURCE_BARRIER *d3d12_barrier = &d3d12_barriers[num_barriers++];
			d3d12_barrier->Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
			d3d12_barrier->Transition.StateBefore = state_before;
			d3d12_barrier->Transition.StateAfter = state_after;
			d3d12_barrier->Transition.pResource = buffer_ptr->buffer;
			d3d12_barrier->Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
			d3d12_barrier->Flags = flags;
		}
		...
		for (uint32_t j = 0; j < opal_barrier->num_texture_transitions; ++j)
		{
			...
			for (uint32_t k = 0; k < texture_view_ptr->num_subresources; ++k)
			{
				D3D12_RESOURCE_STATES state_before = directx12_helperToTextureState(texture_view_ptr->opal_format, texture_view_ptr->usage, opal_transition->state_before);
				D3D12_RESOURCE_STATES state_after = directx12_helperToTextureState(texture_view_ptr->opal_format, texture_view_ptr->usage, opal_transition->state_after);
				...
				d3d12_barrier->Transition.StateBefore = state_before;
				d3d12_barrier->Transition.StateAfter = state_after;
				d3d12_barrier->Transition.pResource = texture_view_ptr->texture;
				d3d12_barrier->Transition.Subresource = texture_view_ptr->subresource_index + k;
				d3d12_barrier->Flags = flags;
			}
		}
	}
	...
	if (num_barriers > 0)
		ID3D12GraphicsCommandList6_ResourceBarrier(command_buffer_ptr->list, num_barriers, d3d12_barriers);
}

static void directx12_cmdMemoryBarrier(DirectX12_Device *device_ptr, DirectX12_CommandBuffer *command_buffer_ptr, const Opal_MemoryBarrierDesc *barriers)
{
	assert(device_ptr);
	assert(command_buffer_ptr);
	assert(command_buffer_ptr->pass != DIRECTX12_PASS_TYPE_NONE);
	...
	uint32_t num_barriers = barriers->num_buffers + barriers->num_textures;
	...
	num_barriers = 0;
	for (uint32_t i = 0; i < barriers->num_buffers; ++i)
	{
		DirectX12_Buffer *buffer_ptr = (DirectX12_Buffer *)opal_poolGetElement(&device_ptr->buffers, (Opal_PoolHandle)barriers->buffers[i]);
		assert(buffer_ptr);

		D3D12_RESOURCE_BARRIER *d3d12_barrier = &d3d12_barriers[num_barriers++];
		d3d12_barrier->Type = D3D12_RESOURCE_BARRIER_TYPE_UAV;
		d3d12_barrier->UAV.pResource = buffer_ptr->buffer;
	}

	for (uint32_t i = 0; i < barriers->num_textures; ++i)
	{
		DirectX12_TextureView *texture_view_ptr = (DirectX12_TextureView *)opal_poolGetElement(&device_ptr->texture_views, (Opal_PoolHandle)barriers->textures[i]);
		assert(texture_view_ptr);

		D3D12_RESOURCE_BARRIER *d3d12_barrier = &d3d12_barriers[num_barriers++];
		d3d12_barrier->Type = D3D12_RESOURCE_BARRIER_TYPE_UAV;
		d3d12_barrier->UAV.pResource = texture_view_ptr->texture;
	}
...
static void directx12_destroyFence(DirectX12_Device *device_ptr, DirectX12_Fence *fence_ptr)
{
	OPAL_UNUSED(device_ptr);
	OPAL_UNUSED(fence_ptr);
...
static void directx12_destroyAccelerationStructure(DirectX12_Device *device_ptr, DirectX12_AccelerationStructure *acceleration_structure_ptr)
{
	...
	ID3D12Resource_Release(acceleration_structure_ptr->buffer);
	directx12_allocatorFreeMemory(device_ptr, acceleration_structure_ptr->allocation);
}

static void directx12_destroyShaderBindingTable(DirectX12_Device *device_ptr, DirectX12_ShaderBindingTable *shader_binding_table_ptr)
{
	assert(device_ptr);
	assert(shader_binding_table_ptr);

	if (shader_binding_table_ptr->buffer != NULL)
	{
		ID3D12Resource_Unmap(shader_binding_table_ptr->buffer, 0, NULL);
		ID3D12Resource_Release(shader_binding_table_ptr->buffer);
		directx12_allocatorFreeMemory(device_ptr, shader_binding_table_ptr->allocation);
	}
}
...
static void directx12_destroyCommandBuffer(DirectX12_Device *device_ptr, DirectX12_CommandBuffer *command_buffer_ptr)
{
	...
	ID3D12GraphicsCommandList6_Release(command_buffer_ptr->list);
}
...
static void directx12_destroyGraphicsPipeline(DirectX12_Device *device_ptr, DirectX12_GraphicsPipeline *pipeline_ptr)
{
	OPAL_UNUSED(device_ptr);
	assert(pipeline_ptr);
...
static void directx12_destroyComputePipeline(DirectX12_Device *device_ptr, DirectX12_ComputePipeline *pipeline_ptr)
{
	OPAL_UNUSED(device_ptr);
	assert(pipeline_ptr);
...
static void directx12_destroyRaytracePipeline(DirectX12_Device *device_ptr, DirectX12_RaytracePipeline *pipeline_ptr)
{
	OPAL_UNUSED(device_ptr);
	assert(pipeline_ptr);
...
static Opal_Result directx12_deviceCreateFence(Opal_Device this, Opal_Fence *fence)
{
	assert(this);
	assert(fence);
...
static Opal_Result directx12_deviceCreateTexture(Opal_Device this, const Opal_TextureDesc *desc, Opal_Texture *texture)
{
	...
	result.allocation = allocation;
	result.usage = desc->usage;
	result.opal_format = desc->format;
...
static Opal_Result directx12_deviceCreateTextureView(Opal_Device this, const Opal_TextureViewDesc *desc, Opal_TextureView *texture_view)
{
	...
	result.num_subresources = desc->layer_count * desc->mip_count;
	...
	result.usage = texture_ptr->usage;
	result.opal_format = texture_ptr->opal_format;
...
static Opal_Result directx12_deviceCreateAccelerationStructure(Opal_Device this, const Opal_AccelerationStructureDesc *desc, Opal_AccelerationStructure *acceleration_structure)
{
	...
	D3D12_RESOURCE_STATES initial_state = D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE;
	Opal_Result opal_result = directx12_createBuffer(device_ptr, &buffer_info, initial_state, OPAL_ALLOCATION_MEMORY_TYPE_DEVICE_LOCAL, OPAL_ALLOCATION_HINT_AUTO, &d3d12_buffer, &allocation);
	if (opal_result != OPAL_SUCCESS)
		return opal_result;
...
static Opal_Result directx12_deviceCreateShaderBindingTable(Opal_Device this, Opal_RaytracePipeline pipeline, Opal_ShaderBindingTable *shader_binding_table)
{
	assert(this);
	assert(pipeline);
	...
	DirectX12_ShaderBindingTable result = {0};
...
static Opal_Result directx12_deviceCreateCommandBuffer(Opal_Device this, Opal_CommandAllocator command_allocator, Opal_CommandBuffer *command_buffer)
{
	...
	ID3D12GraphicsCommandList6 *d3d12_command_list = NULL;
	HRESULT hr = ID3D12Device_CreateCommandList(d3d12_device, 0, command_allocator_ptr->type, d3d12_command_allocator, NULL, &IID_ID3D12GraphicsCommandList6, &d3d12_command_list);
	if (!SUCCEEDED(hr))
		return OPAL_DIRECTX12_ERROR;

	hr = ID3D12GraphicsCommandList6_Close(d3d12_command_list);
	if (!SUCCEEDED(hr))
	{
		ID3D12GraphicsCommandList6_Release(d3d12_command_list);
		return OPAL_DIRECTX12_ERROR;
	}
...
static Opal_Result directx12_deviceCreateComputePipeline(Opal_Device this, const Opal_ComputePipelineDesc *desc, Opal_ComputePipeline *pipeline)
{
	...
	DirectX12_ComputePipeline result = {0};
	result.pipeline_state = d3d12_pipeline_state;

	*pipeline = (Opal_DescriptorSetLayout)opal_poolAddElement(&device_ptr->compute_pipelines, &result);
	return OPAL_SUCCESS;
}
...
static Opal_Result directx12_deviceCreateRaytracePipeline(Opal_Device this, const Opal_RaytracePipelineDesc *desc, Opal_RaytracePipeline *pipeline)
{
	...
	uint32_t max_shaders = desc->num_raygen_functions + desc->num_miss_functions + desc->num_intersection_functions * 3;
	uint32_t max_intersection_groups = desc->num_intersection_functions;
	...
	{
		Opal_PoolHandle anyhit_shader_handle = (Opal_PoolHandle)desc->intersection_functions[i].anyhit_function.shader;
		Opal_PoolHandle closesthit_shader_handle = (Opal_PoolHandle)desc->intersection_functions[i].closesthit_function.shader;
		Opal_PoolHandle intersection_shader_handle = (Opal_PoolHandle)desc->intersection_functions[i].intersection_function.shader;
		...
		{
			assert(desc->intersection_functions[i].anyhit_function.name);

			int size = MultiByteToWideChar(CP_UTF8, 0, desc->intersection_functions[i].anyhit_function.name, -1, NULL, 0);
		...
		{
			assert(desc->intersection_functions[i].closesthit_function.name);

			int size = MultiByteToWideChar(CP_UTF8, 0, desc->intersection_functions[i].closesthit_function.name, -1, NULL, 0);
		...
		{
			assert(desc->intersection_functions[i].intersection_function.name);

			int size = MultiByteToWideChar(CP_UTF8, 0, desc->intersection_functions[i].intersection_function.name, -1, NULL, 0);
	...
	for (uint32_t i = 0; i < desc->num_intersection_functions; ++i)
	{
		DirectX12_Shader *anyhit_shader_ptr = opal_poolGetElement(&device_ptr->shaders, (Opal_PoolHandle)desc->intersection_functions[i].anyhit_function.shader);
		DirectX12_Shader *closesthit_shader_ptr = opal_poolGetElement(&device_ptr->shaders, (Opal_PoolHandle)desc->intersection_functions[i].closesthit_function.shader);
		DirectX12_Shader *intersection_shader_ptr = opal_poolGetElement(&device_ptr->shaders, (Opal_PoolHandle)desc->intersection_functions[i].intersection_function.shader);
		...
		if (anyhit_shader_ptr != NULL)
		{
			...
			int size = *name_lengths++;
			MultiByteToWideChar(CP_UTF8, 0, desc->intersection_functions[i].anyhit_function.name, -1, names, size);
		...
		if (closesthit_shader_ptr != NULL)
		{
			...
			int size = *name_lengths++;
			MultiByteToWideChar(CP_UTF8, 0, desc->intersection_functions[i].closesthit_function.name, -1, names, size);
		...
		if (intersection_shader_ptr != NULL)
		{
			...
			int size = *name_lengths++;
			MultiByteToWideChar(CP_UTF8, 0, desc->intersection_functions[i].intersection_function.name, -1, names, size);
	...
	uint8_t *handles = (uint8_t *)malloc((desc->num_raygen_functions + desc->num_miss_functions + desc->num_intersection_functions) * D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
	...
	DirectX12_RaytracePipeline result = {0};
	result.state_object = d3d12_state_object;
	result.num_raygen_handles = desc->num_raygen_functions;
	result.num_intersection_handles = desc->num_intersection_functions;
	result.num_miss_handles = desc->num_miss_functions;
	result.shader_handles = handles;

	*pipeline = (Opal_DescriptorSetLayout)opal_poolAddElement(&device_ptr->raytrace_pipelines, &result);
	return OPAL_SUCCESS;
}
...
static Opal_Result directx12_deviceCreateSwapchain(Opal_Device this, const Opal_SwapchainDesc *desc, Opal_Swapchain *swapchain)
{
	...
	for (uint32_t i = 0; i < num_textures; ++i)
	{
		...
		result.subresource_index = 0;
		result.num_subresources = 1;
		result.usage = desc->usage;
		result.opal_format = desc->format.texture_format;
...
static Opal_Result directx12_deviceDestroyFence(Opal_Device this, Opal_Fence fence)
{
	assert(this);
	assert(fence);
	...
	DirectX12_Device *device_ptr = (DirectX12_Device *)this;
	DirectX12_Fence *fence_ptr = (DirectX12_Fence *)opal_poolGetElement(&device_ptr->fences, handle);
	assert(fence_ptr);

	opal_poolRemoveElement(&device_ptr->fences, handle);

	directx12_destroyFence(device_ptr, fence_ptr);
	return OPAL_SUCCESS;
}
...
static Opal_Result directx12_deviceDestroyShaderBindingTable(Opal_Device this, Opal_ShaderBindingTable shader_binding_table)
{
	assert(this);
	assert(shader_binding_table);
	...
	DirectX12_Device *device_ptr = (DirectX12_Device *)this;
	DirectX12_ShaderBindingTable *shader_binding_table_ptr = (DirectX12_ShaderBindingTable *)opal_poolGetElement(&device_ptr->shader_binding_tables, handle);
	assert(shader_binding_table_ptr);

	opal_poolRemoveElement(&device_ptr->shader_binding_tables, handle);

	directx12_destroyShaderBindingTable(device_ptr, shader_binding_table_ptr);
	return OPAL_SUCCESS;
}
...
static Opal_Result directx12_deviceDestroyCommandBuffer(Opal_Device this, Opal_CommandBuffer command_buffer)
{
	...
	// TODO: remove handle from directx12_CommandPool instance
...
static Opal_Result directx12_deviceDestroyGraphicsPipeline(Opal_Device this, Opal_GraphicsPipeline pipeline)
{
	...
	DirectX12_Device *device_ptr = (DirectX12_Device *)this;
	DirectX12_GraphicsPipeline *pipeline_ptr = (DirectX12_GraphicsPipeline *)opal_poolGetElement(&device_ptr->graphics_pipelines, handle);
	assert(pipeline_ptr);

	opal_poolRemoveElement(&device_ptr->graphics_pipelines, handle);

	directx12_destroyGraphicsPipeline(device_ptr, pipeline_ptr);
	return OPAL_SUCCESS;
}
...
static Opal_Result directx12_deviceDestroyComputePipeline(Opal_Device this, Opal_ComputePipeline pipeline)
{
	...
	DirectX12_Device *device_ptr = (DirectX12_Device *)this;
	DirectX12_ComputePipeline *pipeline_ptr = (DirectX12_ComputePipeline *)opal_poolGetElement(&device_ptr->compute_pipelines, handle);
	assert(pipeline_ptr);

	opal_poolRemoveElement(&device_ptr->compute_pipelines, handle);

	directx12_destroyComputePipeline(device_ptr, pipeline_ptr);
	return OPAL_SUCCESS;
}
...
static Opal_Result directx12_deviceDestroyRaytracePipeline(Opal_Device this, Opal_RaytracePipeline pipeline)
{
	...
	DirectX12_Device *device_ptr = (DirectX12_Device *)this;
	DirectX12_RaytracePipeline *pipeline_ptr = (DirectX12_RaytracePipeline *)opal_poolGetElement(&device_ptr->raytrace_pipelines, handle);
	assert(pipeline_ptr);

	opal_poolRemoveElement(&device_ptr->raytrace_pipelines, handle);

	directx12_destroyRaytracePipeline(device_ptr, pipeline_ptr);
	return OPAL_SUCCESS;
}
...
static Opal_Result directx12_deviceDestroy(Opal_Device this)
{
	...
	{
		uint32_t head = opal_poolGetHeadIndex(&ptr->raytrace_pipelines);
		while (head != OPAL_POOL_HANDLE_NULL)
		{
			DirectX12_RaytracePipeline *pipeline_ptr = (DirectX12_RaytracePipeline *)opal_poolGetElementByIndex(&ptr->raytrace_pipelines, head);
			directx12_destroyRaytracePipeline(ptr, pipeline_ptr);

			head = opal_poolGetNextIndex(&ptr->raytrace_pipelines, head);
		}

		opal_poolShutdown(&ptr->raytrace_pipelines);
	}

	{
		...
		while (head != OPAL_POOL_HANDLE_NULL)
		{
			DirectX12_ComputePipeline *pipeline_ptr = (DirectX12_ComputePipeline *)opal_poolGetElementByIndex(&ptr->compute_pipelines, head);
			directx12_destroyComputePipeline(ptr, pipeline_ptr);
...

	{
		...
		while (head != OPAL_POOL_HANDLE_NULL)
		{
			DirectX12_GraphicsPipeline *pipeline_ptr = (DirectX12_GraphicsPipeline *)opal_poolGetElementByIndex(&ptr->graphics_pipelines, head);
			directx12_destroyGraphicsPipeline(ptr, pipeline_ptr);
...

	{
		...
		while (head != OPAL_POOL_HANDLE_NULL)
		{
			DirectX12_ShaderBindingTable *shader_binding_table_ptr = (DirectX12_ShaderBindingTable *)opal_poolGetElementByIndex(&ptr->shader_binding_tables, head);
			directx12_destroyShaderBindingTable(ptr, shader_binding_table_ptr);
	...
	{
		uint32_t head = opal_poolGetHeadIndex(&ptr->fences);
		while (head != OPAL_POOL_HANDLE_NULL)
		{
			DirectX12_Fence *fence_ptr = (DirectX12_Fence *)opal_poolGetElementByIndex(&ptr->fences, head);
			directx12_destroyFence(ptr, fence_ptr);

			head = opal_poolGetNextIndex(&ptr->fences, head);
		}
...
static Opal_Result directx12_deviceBuildShaderBindingTable(Opal_Device this, Opal_ShaderBindingTable shader_binding_table, const Opal_ShaderBindingTableBuildDesc *desc)
{
	assert(this);
	assert(desc);
	...
	DirectX12_ShaderBindingTable *shader_binding_table_ptr = (DirectX12_ShaderBindingTable *)opal_poolGetElement(&device_ptr->shader_binding_tables, (Opal_PoolHandle)shader_binding_table);
	assert(shader_binding_table_ptr);

	DirectX12_RaytracePipeline *pipeline_ptr = (DirectX12_RaytracePipeline *)opal_poolGetElement(&device_ptr->raytrace_pipelines, (Opal_PoolHandle)shader_binding_table_ptr->pipeline);
	assert(pipeline_ptr);

	uint32_t sbt_raygen_size = alignUp(desc->num_raygen_indices * aligned_handle_size, base_alignment);
	uint32_t sbt_miss_size = alignUp(desc->num_miss_indices * aligned_handle_size, base_alignment);
	uint32_t sbt_intersection_size = alignUp(desc->num_intersection_indices * aligned_handle_size, base_alignment);
	
	uint64_t sbt_buffer_size = sbt_raygen_size + sbt_intersection_size + sbt_miss_size;
	...
	if (shader_binding_table_ptr->buffer_size < sbt_buffer_size)
	{
		...
		buffer_info.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
		buffer_info.Alignment = 0;
		buffer_info.Width = sbt_buffer_size;
		buffer_info.Height = 1;
		buffer_info.DepthOrArraySize = 1;
		buffer_info.MipLevels = 1;
		buffer_info.Format = DXGI_FORMAT_UNKNOWN;
		...
		Opal_Result result = directx12_createBuffer(device_ptr, &buffer_info, initial_state, OPAL_ALLOCATION_MEMORY_TYPE_UPLOAD, OPAL_ALLOCATION_HINT_AUTO, &shader_binding_table_ptr->buffer, &shader_binding_table_ptr->allocation);
		if (result != OPAL_SUCCESS)
			return result;

		HRESULT hr = ID3D12Resource_Map(shader_binding_table_ptr->buffer, 0, NULL, &shader_binding_table_ptr->buffer_ptr);
		if (!SUCCEEDED(hr))
		{
			ID3D12Resource_Release(shader_binding_table_ptr->buffer);
			directx12_allocatorFreeMemory(device_ptr, shader_binding_table_ptr->allocation);
		...
		shader_binding_table_ptr->address = ID3D12Resource_GetGPUVirtualAddress(shader_binding_table_ptr->buffer);
	...
	if (desc->num_raygen_indices > 0)
		shader_binding_table_ptr->raygen_entry = shader_binding_table_ptr->address;

	if (desc->num_miss_indices > 0)
		shader_binding_table_ptr->miss_entry = shader_binding_table_ptr->address + sbt_raygen_size;

	if (desc->num_intersection_indices > 0)
		shader_binding_table_ptr->intersection_entry = shader_binding_table_ptr->address + sbt_raygen_size + sbt_miss_size;
		...
		for (uint32_t i = 0; i < num_resources; ++i)
		{
			...
			switch (info->opal_type)
			{
				...
				case OPAL_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE:
				{
					...
					desc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
					desc.RaytracingAccelerationStructure.Location = acceleration_structure_ptr->address;
...
static Opal_Result directx12_deviceBeginCommandBuffer(Opal_Device this, Opal_CommandBuffer command_buffer)
{
	...
	HRESULT hr = ID3D12GraphicsCommandList6_Reset(command_buffer_ptr->list, command_allocator_ptr->allocator, NULL);
	if (!SUCCEEDED(hr))
		return OPAL_DIRECTX12_ERROR;
...
static Opal_Result directx12_deviceEndCommandBuffer(Opal_Device this, Opal_CommandBuffer command_buffer)
{
	...
	HRESULT hr = ID3D12GraphicsCommandList6_Close(command_buffer_ptr->list);
	if (!SUCCEEDED(hr))
		return OPAL_DIRECTX12_ERROR;
...
static Opal_Result directx12_deviceCmdSetDescriptorHeap(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_DescriptorHeap descriptor_heap)
{
	...
	ID3D12GraphicsCommandList6_SetDescriptorHeaps(command_buffer_ptr->list, num_heaps, heaps);
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdBeginGraphicsPass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_FramebufferDesc *framebuffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	assert(framebuffer);
	...
	if (barriers != NULL)
		directx12_cmdResourceBarrier(device_ptr, command_buffer_ptr, barriers);
	...
	if (framebuffer->num_color_attachments > 0)
	{
		...
		for (uint32_t i = 0; i < framebuffer->num_color_attachments; ++i)
		{
			const Opal_FramebufferAttachment *opal_attachment = &framebuffer->color_attachments[i];
	...
	{
		const Opal_FramebufferAttachment *opal_attachment = framebuffer->depth_stencil_attachment;
	...
	ID3D12GraphicsCommandList6_BeginRenderPass(command_buffer_ptr->list, framebuffer->num_color_attachments, color_ptrs, depth_stencil_ptr, D3D12_RENDER_PASS_FLAG_ALLOW_UAV_WRITES);
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdGraphicsSetPipelineLayout(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_PipelineLayout pipeline_layout)
{
	assert(this);
	assert(command_buffer);
	assert(pipeline_layout);
	...
	DirectX12_CommandBuffer *command_buffer_ptr = (DirectX12_CommandBuffer *)opal_poolGetElement(&device_ptr->command_buffers, (Opal_PoolHandle)command_buffer);
	assert(command_buffer_ptr);
	assert(command_buffer_ptr->pass == DIRECTX12_PASS_TYPE_GRAPHICS);
	...
	ID3D12GraphicsCommandList6_SetGraphicsRootSignature(d3d12_list, pipeline_layout_ptr->root_signature);

	command_buffer_ptr->pipeline_layout = pipeline_layout;
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdGraphicsSetPipeline(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_GraphicsPipeline pipeline)
{
	assert(this);
	assert(command_buffer);
	assert(pipeline);
...
static Opal_Result directx12_deviceCmdGraphicsSetDescriptorSet(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t index, Opal_DescriptorSet descriptor_set, uint32_t num_dynamic_offsets, const uint32_t *dynamic_offsets)
{
	assert(this);
	assert(command_buffer);
	...
	if (descriptor_set_ptr->resource_handle.ptr > 0)
	{
		...
		ID3D12GraphicsCommandList6_SetGraphicsRootDescriptorTable(d3d12_list, table_index, descriptor_set_ptr->resource_handle);
	}
	...
	if (descriptor_set_ptr->sampler_handle.ptr > 0)
	{
		...
		ID3D12GraphicsCommandList6_SetGraphicsRootDescriptorTable(d3d12_list, table_index, descriptor_set_ptr->sampler_handle);
	}
		...
		for (uint32_t i = 0; i < num_dynamic_offsets; ++i)
		{
			...
			switch (info->api_type)
			{
				case D3D12_DESCRIPTOR_RANGE_TYPE_CBV:
				{
					...
					ID3D12GraphicsCommandList6_SetGraphicsRootConstantBufferView(d3d12_list, parameter_index, address);
				}
				break;
				...
				case D3D12_DESCRIPTOR_RANGE_TYPE_SRV:
				{
					...
					D3D12_GPU_VIRTUAL_ADDRESS address = buffer_ptr->address + buffer_view.offset + dynamic_offsets[i];
					ID3D12GraphicsCommandList6_SetGraphicsRootShaderResourceView(d3d12_list, parameter_index, address);
				}
				break;
				...
				case D3D12_DESCRIPTOR_RANGE_TYPE_UAV:
				{
					...
					D3D12_GPU_VIRTUAL_ADDRESS address = buffer_ptr->address + buffer_view.offset + dynamic_offsets[i];
					ID3D12GraphicsCommandList6_SetGraphicsRootUnorderedAccessView(d3d12_list, parameter_index, address);
				}
				break;
...
static Opal_Result directx12_deviceCmdGraphicsSetIndexBuffer(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_IndexBufferView index_buffer)
{
	assert(this);
	assert(command_buffer);
	...
	ID3D12GraphicsCommandList6_IASetIndexBuffer(command_buffer_ptr->list, &buffer_view);
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdGraphicsSetViewport(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_Viewport viewport)
{
	assert(this);
	assert(command_buffer);
	...
	ID3D12GraphicsCommandList6_RSSetViewports(command_buffer_ptr->list, 1, &d3d12_viewport);
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdGraphicsSetScissor(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t x, uint32_t y, uint32_t width, uint32_t height)
{
	assert(this);
	assert(command_buffer);
	...
	ID3D12GraphicsCommandList6_RSSetScissorRects(command_buffer_ptr->list, 1, &rect);
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdGraphicsDraw(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t num_vertices, uint32_t num_instances, uint32_t base_vertex, uint32_t base_instance)
{
	assert(this);
	assert(command_buffer);
	...
	ID3D12GraphicsCommandList6_DrawInstanced(command_buffer_ptr->list, num_vertices, num_instances, base_vertex, base_instance);
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdGraphicsDrawIndexed(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t num_indices, uint32_t num_instances, uint32_t base_index, int32_t vertex_offset, uint32_t base_instance)
{
	assert(this);
	assert(command_buffer);
	...
	ID3D12GraphicsCommandList6_DrawIndexedInstanced(command_buffer_ptr->list, num_indices, num_instances, base_index, vertex_offset, base_instance);
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdGraphicsMeshletDispatch(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t num_threadgroups_x, uint32_t num_threadgroups_y, uint32_t num_threadgroups_z)
{
	assert(this);
	assert(command_buffer);
	...
	ID3D12GraphicsCommandList6_DispatchMesh(command_buffer_ptr->list, num_threadgroups_x, num_threadgroups_y, num_threadgroups_z);
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdEndGraphicsPass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	...
	ID3D12GraphicsCommandList6_EndRenderPass(command_buffer_ptr->list);
...
static Opal_Result directx12_deviceCmdBeginComputePass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result directx12_deviceCmdComputeSetPipelineLayout(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_PipelineLayout pipeline_layout)
{
	assert(this);
	assert(command_buffer);
	assert(pipeline_layout);
	...
	DirectX12_CommandBuffer *command_buffer_ptr = (DirectX12_CommandBuffer *)opal_poolGetElement(&device_ptr->command_buffers, (Opal_PoolHandle)command_buffer);
	assert(command_buffer_ptr);
	assert(command_buffer_ptr->pass == DIRECTX12_PASS_TYPE_COMPUTE);
	...
	ID3D12GraphicsCommandList6_SetComputeRootSignature(d3d12_list, d3d12_root_signature);

	command_buffer_ptr->pipeline_layout = pipeline_layout;
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdComputeSetPipeline(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_ComputePipeline pipeline)
{
	assert(this);
	assert(command_buffer);
	assert(pipeline);
	...
	DirectX12_ComputePipeline *pipeline_ptr = (DirectX12_ComputePipeline *)opal_poolGetElement(&device_ptr->compute_pipelines, (Opal_PoolHandle)pipeline);
	assert(pipeline_ptr);
...
static Opal_Result directx12_deviceCmdComputeSetDescriptorSet(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t index, Opal_DescriptorSet descriptor_set, uint32_t num_dynamic_offsets, const uint32_t *dynamic_offsets)
{
	assert(this);
	assert(command_buffer);
	...
	if (descriptor_set_ptr->resource_handle.ptr > 0)
	{
		...
		ID3D12GraphicsCommandList6_SetComputeRootDescriptorTable(d3d12_list, table_index, descriptor_set_ptr->resource_handle);
	}
	...
	if (descriptor_set_ptr->sampler_handle.ptr > 0)
	{
		...
		ID3D12GraphicsCommandList6_SetComputeRootDescriptorTable(d3d12_list, table_index, descriptor_set_ptr->sampler_handle);
	}
		...
		for (uint32_t i = 0; i < num_dynamic_offsets; ++i)
		{
			...
			switch (info->api_type)
			{
				case D3D12_DESCRIPTOR_RANGE_TYPE_CBV:
				{
					...
					D3D12_GPU_VIRTUAL_ADDRESS address = buffer_ptr->address + buffer_view.offset + dynamic_offsets[i];
					ID3D12GraphicsCommandList6_SetComputeRootConstantBufferView(d3d12_list, parameter_index, address);
				}
				break;
				...
				case D3D12_DESCRIPTOR_RANGE_TYPE_SRV:
				{
					...
					D3D12_GPU_VIRTUAL_ADDRESS address = buffer_ptr->address + buffer_view.offset + dynamic_offsets[i];
					ID3D12GraphicsCommandList6_SetComputeRootShaderResourceView(d3d12_list, parameter_index, address);
				}
				break;
				...
				case D3D12_DESCRIPTOR_RANGE_TYPE_UAV:
				{
					...
					D3D12_GPU_VIRTUAL_ADDRESS address = buffer_ptr->address + buffer_view.offset + dynamic_offsets[i];
					ID3D12GraphicsCommandList6_SetComputeRootUnorderedAccessView(d3d12_list, parameter_index, address);
				}
				break;
...
static Opal_Result directx12_deviceCmdComputeMemoryBarrier(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_MemoryBarrierDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	assert(barriers);
	...
	directx12_cmdMemoryBarrier(device_ptr, command_buffer_ptr, barriers);
	return OPAL_SUCCESS;
}
...
static Opal_Result directx12_deviceCmdComputeDispatch(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t num_threadgroups_x, uint32_t num_threadgroups_y, uint32_t num_threadgroups_z)
{
	...
	ID3D12GraphicsCommandList6_Dispatch(command_buffer_ptr->list, num_threadgroups_x, num_threadgroups_y, num_threadgroups_z);
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdEndComputePass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result directx12_deviceCmdBeginRaytracePass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	...
	command_buffer_ptr->pass = DIRECTX12_PASS_TYPE_RAYTRACE;
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdRaytraceSetPipelineLayout(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_PipelineLayout pipeline_layout)
{
	assert(this);
	assert(command_buffer);
	assert(pipeline_layout);
	...
	DirectX12_CommandBuffer *command_buffer_ptr = (DirectX12_CommandBuffer *)opal_poolGetElement(&device_ptr->command_buffers, (Opal_PoolHandle)command_buffer);
	assert(command_buffer_ptr);
	assert(command_buffer_ptr->pass == DIRECTX12_PASS_TYPE_RAYTRACE);
...
static Opal_Result directx12_deviceCmdRaytraceSetPipeline(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_RaytracePipeline pipeline)
{
	assert(this);
	assert(command_buffer);
	assert(pipeline);
	...
	DirectX12_RaytracePipeline *pipeline_ptr = (DirectX12_RaytracePipeline *)opal_poolGetElement(&device_ptr->raytrace_pipelines, (Opal_PoolHandle)pipeline);
	assert(pipeline_ptr);

	ID3D12GraphicsCommandList6 *d3d12_list = command_buffer_ptr->list;

	ID3D12GraphicsCommandList6_SetPipelineState1(d3d12_list, pipeline_ptr->state_object);

	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdRaytraceSetDescriptorSet(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t index, Opal_DescriptorSet descriptor_set, uint32_t num_dynamic_offsets, const uint32_t *dynamic_offsets)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result directx12_deviceCmdRaytraceSetShaderBindingTable(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_ShaderBindingTable shader_binding_table)
{
	assert(this);
	assert(command_buffer);
	assert(shader_binding_table);
	...
	DirectX12_ShaderBindingTable *shader_binding_table_ptr = (DirectX12_ShaderBindingTable *)opal_poolGetElement(&device_ptr->shader_binding_tables, (Opal_PoolHandle)shader_binding_table);
	assert(shader_binding_table_ptr);
	assert(shader_binding_table_ptr->buffer != NULL);
...
static Opal_Result directx12_deviceCmdRaytraceMemoryBarrier(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_MemoryBarrierDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	assert(barriers);
...
static Opal_Result directx12_deviceCmdRaytraceDispatch(Opal_Device this, Opal_CommandBuffer command_buffer, uint32_t width, uint32_t height, uint32_t depth)
{
	assert(this);
	assert(command_buffer);
	...
	{
		assert(isAlignedul(command_buffer_ptr->raygen_entry, D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT));

		desc.RayGenerationShaderRecord.StartAddress = command_buffer_ptr->raygen_entry;
		desc.RayGenerationShaderRecord.SizeInBytes = aligned_handle_size;
	}

	if (command_buffer_ptr->miss_entry > 0)
	{
		assert(isAlignedul(command_buffer_ptr->miss_entry, D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT));

		desc.MissShaderTable.StartAddress = command_buffer_ptr->miss_entry;
	...
	{
		assert(isAlignedul(command_buffer_ptr->intersection_entry, D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT));

		desc.HitGroupTable.StartAddress = command_buffer_ptr->intersection_entry;
		desc.HitGroupTable.SizeInBytes = aligned_handle_size;
		desc.HitGroupTable.StrideInBytes = aligned_handle_size;
	}

	ID3D12GraphicsCommandList6_DispatchRays(command_buffer_ptr->list, &desc);
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdEndRaytracePass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result directx12_deviceCmdBeginCopyPass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	...
	command_buffer_ptr->pass = DIRECTX12_PASS_TYPE_COPY;
	return OPAL_SUCCESS;
}
...
static Opal_Result directx12_deviceCmdCopyBufferToBuffer(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_Buffer src_buffer, uint64_t src_offset, Opal_Buffer dst_buffer, uint64_t dst_offset, uint64_t size)
{
	...
	DirectX12_CommandBuffer *command_buffer_ptr = (DirectX12_CommandBuffer *)opal_poolGetElement(&device_ptr->command_buffers, (Opal_PoolHandle)command_buffer);
	assert(command_buffer_ptr);
	assert(command_buffer_ptr->pass == DIRECTX12_PASS_TYPE_COPY);
	...
	ID3D12GraphicsCommandList6_CopyBufferRegion(command_buffer_ptr->list, dst_buffer_ptr->buffer, dst_offset, src_buffer_ptr->buffer, src_offset, size);
	return OPAL_SUCCESS;
}
...
static Opal_Result directx12_deviceCmdCopyBufferToTexture(Opal_Device this, Opal_CommandBuffer command_buffer, Opal_BufferTextureRegion src, Opal_TextureRegion dst, Opal_Extent3D size)
{
	...
	ID3D12GraphicsCommandList6_CopyTextureRegion(
...
static Opal_Result directx12_deviceCmdEndCopyPass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
...
static Opal_Result directx12_deviceCmdBeginAccelerationStructurePass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
	...
	command_buffer_ptr->pass = DIRECTX12_PASS_TYPE_ACCELERATION_STRUCTURE;
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdAccelerationStructureBuild(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_AccelerationStructureBuildDesc *desc)
{
	assert(this);
	assert(command_buffer);
	assert(desc);
	...
	DirectX12_CommandBuffer *command_buffer_ptr = (DirectX12_CommandBuffer *)opal_poolGetElement(&device_ptr->command_buffers, (Opal_PoolHandle)command_buffer);
	assert(command_buffer_ptr);
	assert(command_buffer_ptr->pass == DIRECTX12_PASS_TYPE_ACCELERATION_STRUCTURE);
	...
	ID3D12GraphicsCommandList6_BuildRaytracingAccelerationStructure(command_buffer_ptr->list, &build_desc, 0, NULL);

	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdAccelerationStructureCopy(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_AccelerationStructureCopyDesc *desc)
{
	assert(this);
	assert(command_buffer);
	assert(desc);
	...
	ID3D12GraphicsCommandList6_CopyRaytracingAccelerationStructure(command_buffer_ptr->list, dst_address, src_address, copy_mode);
	return OPAL_SUCCESS;
}

static Opal_Result directx12_deviceCmdEndAccelerationStructurePass(Opal_Device this, Opal_CommandBuffer command_buffer, const Opal_PassBarriersDesc *barriers)
{
	assert(this);
	assert(command_buffer);
...
static Opal_DeviceTable device_vtbl =
{
	...
	directx12_deviceCmdGraphicsSetPipelineLayout,
	directx12_deviceCmdGraphicsSetPipeline,
	directx12_deviceCmdGraphicsSetDescriptorSet,
	directx12_deviceCmdGraphicsSetVertexBuffers,
	directx12_deviceCmdGraphicsSetIndexBuffer,
	directx12_deviceCmdGraphicsSetViewport,
	directx12_deviceCmdGraphicsSetScissor,
	directx12_deviceCmdGraphicsDraw,
	directx12_deviceCmdGraphicsDrawIndexed,
	directx12_deviceCmdGraphicsMeshletDispatch,
	directx12_deviceCmdEndGraphicsPass,

	directx12_deviceCmdBeginComputePass,
	directx12_deviceCmdComputeSetPipelineLayout,
	directx12_deviceCmdComputeSetPipeline,
	directx12_deviceCmdComputeSetDescriptorSet,
	directx12_deviceCmdComputeMemoryBarrier,
	directx12_deviceCmdComputeDispatch,
	directx12_deviceCmdEndComputePass,

	directx12_deviceCmdBeginRaytracePass,
	directx12_deviceCmdRaytraceSetPipelineLayout,
	directx12_deviceCmdRaytraceSetPipeline,
	directx12_deviceCmdRaytraceSetDescriptorSet,
	directx12_deviceCmdRaytraceSetShaderBindingTable,
	directx12_deviceCmdRaytraceMemoryBarrier,
	...
	directx12_deviceCmdBeginAccelerationStructurePass,
...
Opal_Result directx12_deviceInitialize(DirectX12_Device *device_ptr, DirectX12_Instance *instance_ptr, IDXGIAdapter1 *adapter, ID3D12Device *device)
{
	...
	opal_poolInitialize(&device_ptr->fences, sizeof(DirectX12_Fence), 32);
	...
	opal_poolInitialize(&device_ptr->shader_binding_tables, sizeof(DirectX12_ShaderBindingTable), 32);
	...
	opal_poolInitialize(&device_ptr->graphics_pipelines, sizeof(DirectX12_GraphicsPipeline), 32);
	opal_poolInitialize(&device_ptr->compute_pipelines, sizeof(DirectX12_ComputePipeline), 32);
	opal_poolInitialize(&device_ptr->raytrace_pipelines, sizeof(DirectX12_RaytracePipeline), 32);
