...
D3D12_RESOURCE_STATES directx12_helperToBufferState(Opal_AllocationMemoryType type, Opal_BufferUsageFlags usage, Opal_BufferState state)
{
	...
	switch (state)
	{
		case OPAL_BUFFER_STATE_UNORDERED_ACCESS: return D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
...
D3D12_RESOURCE_FLAGS directx12_helperToBufferFlags(Opal_AllocationMemoryType type, Opal_BufferUsageFlags usage)
{
	...
	if ((type == OPAL_ALLOCATION_MEMORY_TYPE_DEVICE_LOCAL) && (usage & OPAL_BUFFER_USAGE_UNORDERED_ACCESS))
...
D3D12_RESOURCE_STATES directx12_helperToTextureState(Opal_TextureFormat format, Opal_TextureUsageFlags usage, Opal_TextureState state)
{
	switch (state)
	{
		case OPAL_TEXTURE_STATE_UNDEFINED: return D3D12_RESOURCE_STATE_COMMON;
		case OPAL_TEXTURE_STATE_PRESENT: return D3D12_RESOURCE_STATE_PRESENT;
		...
		case OPAL_TEXTURE_STATE_SHADER_SAMPLED:
		{
			...
			assert(result != D3D12_RESOURCE_STATE_COMMON);
			return result;
		}
...
{
	static D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE d3d12_types[] =
	{
...
{
	static D3D12_RAYTRACING_GEOMETRY_TYPE d3d12_types[] =
	{
...
{
	static D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE d3d12_modes[] =
	{
