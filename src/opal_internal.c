...
uint32_t opalEvaluateDevice(const Opal_DeviceInfo *info, Opal_DeviceHint hint)
{
	...
	switch (hint)
	{
		case OPAL_DEVICE_HINT_PREFER_HIGH_PERFORMANCE: score += gpu_type_high_performance_scores[info->gpu_type];
		case OPAL_DEVICE_HINT_PREFER_LOW_POWER: score += gpu_type_low_power_scores[info->gpu_type];
		default: score += gpu_type_default_scores[info->gpu_type];
	}
